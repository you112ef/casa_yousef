...
export default function FileDropzone({ accept, onFile, capture }: { accept?: string; capture?: string; onFile: (file: File) => void }) {
  ...
  return (
    ...
    >
      <input ref={inputRef} type="file" accept={accept} {...(capture ? { capture } : {})} hidden onChange={(e) => { const f = e.target.files?.[0]; if (f) onFile(f); }} />
